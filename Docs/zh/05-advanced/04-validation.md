...
|**抽取**|`ExcelColumnExtractor`|Excel **单元格值** 是否与架构和 Attribute 兼容|
|**加载**|`StaticDataManager.LoadAsync`|运行时加载时点的结构、值与引用检查|

声明检查和抽取在构建流水线（离线）中运行，加载在应用程序运行时运行。由于声明检查分析 `.cs` 源代码，因此在 `ExcelColumnExtractor` 和 `StaticDataHeaderGenerator` 任一运行时都会执行，而在没有源代码的运行时则不会运行。
...
|`[StaticDataRecord]` 的存在|O|O|O|识别声明检查对象 / 抽取对象为 0 个时终止 / 加载时必需|
...
在单元格值检查（C）中，`[Range]`·`[RegularExpression]`·`[CountRange]` 在抽取和加载两端以相同规则运行。但 Primary Key 重复以及声明一致性检查（A·B）仅在抽取阶段（含声明检查）执行。如果将未经抽取阶段的 CSV 直接投入运行时，这些检查会被跳过，因此健康的流水线始终只将经过抽取器的 CSV 上线运营。
