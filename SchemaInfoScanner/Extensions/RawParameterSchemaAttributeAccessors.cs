...
public static class RawParameterSchemaAttributeAccessors
{
    public static bool HasAttribute<T>(
        this RawParameterSchema parameterSchema)
        where T : Attribute
    {
    ...
    public static IReadOnlyList<string> GetAttributeValueList<TAttribute>(this RawParameterSchema parameterSchema)
    {
        var attributeName = typeof(TAttribute).Name.Replace("Attribute", string.Empty);
        var attribute = parameterSchema.AttributeList.Single(x => x.Name.ToString() == attributeName);
