...
public static class GenerateLengthFileHandler
{
    ...
    public static int Generate(GenerateLengthFileOptions options)
    {
        ...
        var recordContainerInfo = new RecordContainerInfo(targetRecordSchema.RecordName.FullName, lengthRequiredNames);

        IniWriter.Write(options.OutputPath, new() { recordContainerInfo });
        LogInformation(logger, "GenerateLengthFile is done.", null);
